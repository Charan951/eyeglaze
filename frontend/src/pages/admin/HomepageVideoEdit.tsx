...
const LIST_PATH = '/admin/homepage-videos';

export default function HomepageVideoEdit() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (isNew) return;
    setLoading(true);
    api.get(`/admin/homepage-videos/${id}`)
      .then((res) => {
        ...
        setInputMode(isDirectVideo(video.videoUrl || '') ? 'upload' : 'url');
        setError('');
      })
      .catch((err: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isNew) {
        ...
        await api.put(`/admin/homepage-videos/${id}`, payload);
      }
      setSuccess(isNew ? 'Video added successfully!' : 'Video updated successfully!');
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white">{isNew ? 'Add Video' : 'Edit Video'}</h1>
        <p className="text-[#A7A7A7] text-xs mt-1">
          Videos appear beside Frequently Asked Questions on the landing page.
        </p>
      </div>
